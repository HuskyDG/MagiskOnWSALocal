...
if magisk_ver == "stable" or magisk_ver == "beta" or magisk_ver == "canary" or magisk_ver == "debug":
    try:
        magisk_link = json.loads(requests.get(
            f"https://github.com/huskydg/magisk-files/raw/main/{magisk_ver}.json").content)['magisk']['link']
    except Exception:
        print("Failed to fetch from GitHub API, fallbacking to jsdelivr...")
        magisk_link = json.loads(requests.get(
            f"https://fastly.jsdelivr.net/gh/huskydg/magisk-files@main/{magisk_ver}.json").content)['magisk']['link']
